<?php require_once 'mainHeader.php';?>
<?php require_once 'subHeader02.php';?>
<style>#panel-4 .thumbnail-overlay{display: block;}</style>

<div class="frame">

  <div class="bit-1">
    <div class="box" id="description">
      <p><span class="titlePG">Pixelxpixel.net</span> is a mock-phishing website to promote awareness to internet security. One click is all that it takes to compromise your security on the internet.</p>
      <p class="titleSub">Web Art<span class="yellow"> . </span>Side Project<span class="yellow"> . </span>2014</p>
    </div>
  </div>

  <div class="bit-1">
    <div class="project-btn" id="project">
      <a href="http://pixelxpixel.juliomontas.com/" target="_blank">Visit the site</a>
    </div>
  </div>

  <div class="bit-1">
    <div class="work01" id="project">
      <p>The first version's concept was to have just one still pixel on the screen</p>
      <img class="projectTab" style="border-style: none;" src='img/assets/browserTab.png'>
      <img style="border-style: none;" src='img/projects/PixelXPixel/pixelXPixel01.gif'>
    </div>
  </div>

  <div class="bit-1">
    <div class="work02" id="project">
      <p>One click prompts an alarming reaction</p>
      <img class="projectTab" style="border-style: none;" src='img/assets/browserTab.png'>
      <img style="border-style: none;" src='img/projects/PixelXPixel/Pixel_X_Pixel_V02_Click.gif'>
    </div>
  </div>

  <div class="bit-1">
    <div class="work03" id="project">
      <p>The mock-phishing website is meant to make users think twice before clicking any suspicious links</p>
      <img class="projectTab" style="border-style: none;" src='img/assets/browserTab.png'>
      <img style="border-style: none;" src='img/projects/PixelXPixel/Pixel_X_Pixel_V02_Menu.gif'>
    </div>
  </div>

</div>
<?php require_once("projectFooter.php"); ?>
